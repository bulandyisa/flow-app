...
- **first**: композиция клипа — единственный кадр, который генерируется через NB Pro
- Видео (VEO) генерируется напрямую из первого кадра, без последнего кадра

### Chain: first → VEO
```
first (NB Pro) → VEO (видео из первого кадра)
```

Принятый first кадр передаётся в VEO как начальный кадр видео.
...
## Character Performance Bible (Актёрская игра персонажей)

Каждый персонаж выражает эмоции по-своему. Недостаточно написать `surprised expression` —
нужно описать, КАК ИМЕННО этот персонаж удивляется.

### Принцип:
Перед началом генерации промптов для фильма — составить Performance Bible для каждого персонажа.

### Формат:
```
[Имя персонажа] — [тип: интроверт/экстраверт/...]

Удивление: [конкретное физическое проявление]
Интерес: [конкретное физическое проявление]
Грусть: [конкретное физическое проявление]
Радость: [конкретное физическое проявление]
Тревога: [конкретное физическое проявление]

Характерные жесты: [что делает руками, как стоит, как ходит]
Чего НИКОГДА не делает: [не жестикулирует широко / не улыбается открыто / ...]
```

### Правила:
1. **Интроверт ≠ Экстраверт.** Интроверт замирает от удивления, экстраверт — вскидывается.
2. **Эмоция через тело, не только лицо.** `shoulders drop, gaze drifts down` > `sad expression`.
3. **Контраст = характер.** Если обычно шумный персонаж замолкает — это сильнее, чем крик.
4. **Performance Bible передаётся Claude** вместе с правилами при генерации промптов.

---

## Compositional Depth (Глубина композиции)

### Три плана в каждом кадре (кроме крупных):
- **Передний план:** объект, создающий глубину (край стола, дверной проём, рука, предмет)
- **Средний план:** основное действие (персонаж + его действие)
- **Задний план:** контекст (стена, окно, другой персонаж)

### Framing (обрамление):
Использовать элементы окружения для обрамления персонажа:
- `framed through the doorway` — изоляция, одиночество, наблюдение
- `framed against open window` — стремление, мечта, свобода
- `framed between shelves` — замкнутость, рутина
- `silhouetted in the doorway` — загадочность, появление

### Leading lines (ведущие линии):
Объекты в кадре направляют взгляд зрителя:
- Край стола → к персонажу
- Провода/трубы → к точке действия
- Взгляд одного персонажа → к другому персонажу
- Луч света → к ключевому объекту

### Как использовать в промптах:
Не описывай композицию абстрактно (`good composition`).
Описывай через action anchors и camera angle:
```
Good: "The exact character from Image 1 leans through the doorway, 
watching. Medium shot from inside the room, 35mm lens."
Bad: "Well-composed shot with foreground framing elements."
```

---

## Emotion Intensity Scale (Шкала интенсивности эмоций)

### Проблема: `surprised expression` слишком абстрактно для AI.

### Шкала 1-10:
| Уровень | Проявление | Какой план нужен |
|---------|-----------|-----------------|
| 1-3 | Микровыражение: только лицо (глаза чуть расширяются, уголок рта) | Close-up |
| 4-6 | Видимая эмоция: лицо + тело (наклон головы, плечи, жест) | Medium shot |
| 7-9 | Сильная эмоция: всё тело (встаёт, отступает, замирает целиком) | Medium/Wide |
| 10 | Экстрим: плач, крик, падение (ТОЛЬКО для кульминации) | Любой |

### Как описывать в промптах:

**Вместо абстрактных ярлыков — конкретные физические проявления:**

| Абстрактно (ПЛОХО) | Конкретно (ХОРОШО) |
|---------------------|---------------------|
| `surprised expression` | `eyes widen slightly, body freezes mid-motion` (3/10) |
| `sad` | `shoulders drop, gaze drifts downward, still` (4/10) |
| `happy` | `leans forward with bright eyes, slight smile` (5/10) |
| `angry` | `jaw tightens, hands grip the edge of the table` (6/10) |
| `terrified` | `stumbles backward, hands raised defensively` (8/10) |

### Правила:
1. **Интенсивность должна соответствовать сценарию.** Не раздувай эмоцию.
2. **Крупный план = тонкая эмоция (1-4).** Широкий план = крупная эмоция (6-9).
3. **Смешанные эмоции сильнее чистых:** `relief mixed with lingering worry` > `relieved`.
4. **Привязывай эмоцию к Character Performance Bible** — каждый персонаж выражает по-своему.
