...
> Режим: `image_to_video` (видео генерируется из первого кадра)
> Модель: `Veo 3.1 - Fast`
...
В режиме image-to-video VEO **видит первый кадр**. Он знает, как начинается сцена.
Промпт должен описывать **ЧТО ПРОИСХОДИТ** в видео, начиная с этого кадра.

**Два источника информации для VEO:**

| Источник | Что контролирует |
|----------|-----------------|
| First frame | Начальная композиция, персонажи, обстановка |
| Текст промпта | Движение, камера, действие, аудио, стиль |

**Золотое правило: НЕ переописывай то, что видно на кадре.**
VEO уже видит кадр — дублирование текстом создаёт конфликт и нестабильность.

---

## Режим VEO

### «Видео из первого кадра» (frames) — default
- Загружает first frame в слот кадра
- VEO генерирует видео, начиная с этого кадра
- Промпт описывает действие/анимацию

**Режим:** задаётся полем `veo_mode` в JSON-конфиге клипа. Default = `"frames"`.
...
### Формула:
```
[CAMERA MOVEMENT] → [ACTION] → [AUDIO] → [STYLE]
```

### 4 компонента:

### 1. Camera Movement (ДВИЖЕНИЕ КАМЕРЫ)
**Самый важный компонент.** Камера определяет визуальную динамику видео.
...
### 2. Action (ДЕЙСТВИЕ)
Описывай **движение и действие**, а НЕ статические позы.
...
Фокусируйся на ОДНОМ главном действии в видео.
...
| Начальную позу | VEO видит first frame | Описывай действие |
| Одежду, цвет | VEO видит на кадре | Ничего |

**Исключение:** если действие неочевидно из кадра, можно кратко уточнить контекст.
