...
# Use FlowData venv if available (avoids iCloud-evicted main venv)
if [ -x "$HOME/FlowData/pw_venv/bin/python3" ]; then
    PYTHON="$HOME/FlowData/pw_venv/bin/python3"
elif [ -x "/tmp/pw_venv/bin/python3" ]; then
    PYTHON="/tmp/pw_venv/bin/python3"
else
    PYTHON="$PROJECT_DIR/venv/bin/python3"
fi
BOT="$SCRIPT_DIR/flow_bot_v2.py"
TIMEOUT="${FLOW_TIMEOUT:-18000}"
...
# Проверяем наличие --chromium флага для правильной сессии
HAS_CHROMIUM=""
for arg in "$@"; do
    if [ "$arg" = "--chromium" ]; then HAS_CHROMIUM="_chromium"; fi
done

case "$ACCOUNT" in
    1) SESSION_DIR="$PROJECT_DIR/.session${HAS_CHROMIUM}" ;;
    2) SESSION_DIR="$PROJECT_DIR/.session_1b${HAS_CHROMIUM}" ;;
    3) SESSION_DIR="$PROJECT_DIR/.session_2${HAS_CHROMIUM}" ;;
    4) SESSION_DIR="$PROJECT_DIR/.session_2b${HAS_CHROMIUM}" ;;
    5) SESSION_DIR="$PROJECT_DIR/.session_1c${HAS_CHROMIUM}" ;;
    6) SESSION_DIR="$PROJECT_DIR/.session_2c${HAS_CHROMIUM}" ;;
    *) SESSION_DIR="$PROJECT_DIR/.session${HAS_CHROMIUM}" ;;
esac
