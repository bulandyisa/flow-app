...
## Архитектура: 4 бота
Есть 4 бота, каждый со своим аккаунтом Google и проектом Flow:

| Бот | Аккаунт | Сессия | Проект | CLI |
|-----|---------|--------|--------|-----|
| Bot 1 | Акк 1 | `.session` | helper1 | `--account 1` |
| Bot 2 | Акк 1 | `.session_1b` | helper2 | `--account 2` |
| Bot 3 | Акк 2 | `.session_2` | helper3 | `--account 3` |
| Bot 4 | Акк 2 | `.session_2b` | helper4 | `--account 4` |

### Распределение сцен по ботам
Каждый бот обрабатывает свой набор клипов. Не перекрещивать!

- **Bot 1**: S01_A, S01_B, S02_A, S02_B
- **Bot 2**: S02_C, S03_A, S03_B
- **Bot 3**: S04_A, S04_B, S04_C
- **Bot 4**: S05_A, S05_B, S05_C, S05_D

### Важные правила для ботов
1. **НЕ переключать аккаунт при ошибках** — каждый бот работает только на своём аккаунте
2. **Ничего руками не одобряется** — бот полностью автономен: генерирует, скачивает, сохраняет
3. **Retry на server_error** — до 3 попыток с паузами (45s, 60s, 90s)
4. **Content filter** — НЕ повторять (prompt blocked)
...
- **Папа/Мама**: проверять по референсу
