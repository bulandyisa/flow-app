...
def human_click(page = None, selector_or_element = None, timeout = None):
    '''Click with small random offset and delay to mimic human behavior.'''
    human_delay(0.2, 0.6)
...
def run_veo_pass(page = None, clip = None, first_frame = None, last_frame = None):
    '''Pass 3: Animate First Frame + Last Frame using VEO 3.1.

    Steps:
      1. Switch to "Видео по кадрам" mode
      2. Upload first frame (slot 0)
      3. Upload last frame (slot 1)
      4. Fill VEO prompt
      5. Click Generate
      6. Wait for video result
      7. Download the clip

    Returns path to downloaded video clip, or None on failure.
    '''
    clip_id = clip['clip_id']
    print(f'''\n{'============================================================'}''')
    print(f'''  PASS 3 — VEO 3.1 (First+Last Frame) — {clip_id}''')
    print(f'''{'============================================================'}''')
    prompt = sanitize_prompt(clip['veo_prompt'])
    print(f'''  Prompt: {prompt[:80]}...''')
    print(f'''  First frame: {first_frame.name}''')
    print(f'''  Last frame:  {last_frame.name}''')
    save_path = CLIPS_DIR / f'''{clip_id}_clip.mp4'''
    if save_path.exists():
        print(f'''  Clip already exists: {save_path} — skipping generation''')
        return save_path
    ...
    clear_prompt(page)
    fill_prompt(page, prompt)
    take_debug_screenshot(page, f'{clip_id}_veo_before_gen')
    click_generate(page)
    result = wait_for_new_gallery_item(page, initial_urls, tab='Видео')
    if result != 'success':
        take_debug_screenshot(page, f'{clip_id}_veo_timeout')
        print(f'  FAILED: VEO generation for {clip_id} ({result})')
        return None
    take_debug_screenshot(page, f'{clip_id}_veo_after_gen')
    if download_latest_video(page, save_path):
        return save_path
    print(f'  FAILED: could not download video for {clip_id}')
    return None
